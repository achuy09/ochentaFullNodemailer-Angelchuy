...
const resetCorreo = require('../../../../libs/resetpw/indexreset');
const userDao = new UsuarioDao();
const user = new Usuario(userDao);
user.init();

const {jwtSignResetPassword} = require('../../../../libs/resetpw');
...
router.post('/resetPassword', async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({ msj: "Su contraseña se actualizo" });
...
router.post('/recoveryPassword', async (req, res) => {
  try {
    ...
    await resetCorreo.sendEmail(req, res, data);
